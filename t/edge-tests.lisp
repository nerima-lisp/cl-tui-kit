...
(deftest backend-size-returns-independent-copy (:backend)
  (let* ((backend (make-backend :size (make-size 5 3)))
         (first (backend-size backend)))
    (is-equal 5 (size-width first))
    (is-equal 3 (size-height first))
    (setf (size-width first) 9999)
    (setf (size-height first) 9999)
    (let ((second (backend-size backend)))
      (is-equal 5 (size-width second))
      (is-equal 3 (size-height second))
      (is (not (eq first second))))))

(deftest backend-cursor-returns-independent-copy (:backend)
  (let* ((backend (make-backend :cursor (make-point 2 4)))
         (first (backend-cursor backend)))
    (is-equal 2 (point-x first))
    (is-equal 4 (point-y first))
    (setf (point-x first) 9999)
    (setf (point-y first) 9999)
    (let ((second (backend-cursor backend)))
      (is-equal 2 (point-x second))
      (is-equal 4 (point-y second))
      (is (not (eq first second))))))

(deftest backend-mutators-still-update-size-and-cursor-readers (:backend)
  (let ((backend (make-backend :size (make-size 4 2) :cursor (make-point 1 1))))
    (backend-resize backend 10 6)
    (is-equal 10 (size-width (backend-size backend)))
    (is-equal 6 (size-height (backend-size backend)))
...
(deftest focus-node-children-returns-independent-copy (:edge-cases)
  (let* ((child-a (make-focus-node :child-a))
         (child-b (make-focus-node :child-b))
         (root (make-focus-node :root :scope-p t
                                :children (list child-a child-b)))
         (first (focus-node-children root)))
    (is-equal (list child-a child-b) first)
    (setf (car first) :corrupted)
    (is-equal (list child-a child-b) (focus-node-children root))
    (is (not (eq first (focus-node-children root))))))

(deftest focus-tree-modal-stack-returns-independent-copy (:edge-cases)
  (let* ((modal-a (make-focus-node :modal-a :scope-p t))
         (root (make-focus-node :root :scope-p t
                                :children (list modal-a)))
         (tree (make-focus-tree root)))
    (is (null (focus-tree-modal-stack tree)))
    (focus-push-modal tree modal-a)
    (let ((first (focus-tree-modal-stack tree)))
      (is-equal 1 (length first))
      (is (eq modal-a (first (first first))))
      (setf (car first) :corrupted)
      (is-equal 1 (length (focus-tree-modal-stack tree)))
      (is (eq modal-a (first (first (focus-tree-modal-stack tree)))))
      (is (not (eq first (focus-tree-modal-stack tree)))))
    (focus-pop-modal tree)
    (is (null (focus-tree-modal-stack tree)))))

(deftest widget-rectangle-returns-independent-copy (:edge-cases)
  (let* ((widget (make-widget)))
    (widget-layout widget (test-rectangle 1 2 3 4))
    (let ((first (widget-rectangle widget)))
      (is-equal (test-rectangle 1 2 3 4) first)
      (setf (rectangle-x first) 999)
      (is-equal (test-rectangle 1 2 3 4) (widget-rectangle widget))
      (is (not (eq first (widget-rectangle widget)))))))

(deftest widget-children-returns-independent-copy (:edge-cases)
  (let* ((child (make-widget))
         (box (make-box-widget child))
         (first (widget-children box)))
    (is-equal (list child) first)
    (setf (car first) :corrupted)
    (is-equal (list child) (widget-children box))
    (is (not (eq first (widget-children box))))))
