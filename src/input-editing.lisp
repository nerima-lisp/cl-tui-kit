...
(defun %input-move-left (widget value cursor shift-p word-p)
...
(defun %input-move-right (widget value cursor shift-p word-p)
  (if (and (not shift-p) (%input-has-selection-p widget))
      (multiple-value-bind (start end) (%input-selection-range widget)
        (declare (ignore start))
...
(defun %input-handle-cursor-key (widget event)
  (let* ((key (key-event-key event))
    ...
    (cond
      ((equalp key :left)
       (%input-move-left widget value cursor shift-p word-p))
      ((equalp key :right)
       (%input-move-right widget value cursor shift-p word-p))
      ((or (equalp key :home)
...
(defun %input-handle-history-key (widget event)
  (let ((key (key-event-key event))
        (control-p (%input-modifier-p event :ctrl))
        (shift-p (%input-modifier-p event :shift))
        (alt-p (%input-modifier-p event :alt)))
...
(defun %input-handle-key-event (widget event)
  (or (%input-handle-cursor-key widget event)
      (%input-handle-history-key widget event)
      (%input-handle-delete-key widget event)
      (let ((key (key-event-key event)))
        (cond
...
(defmethod widget-handle-event ((widget input-widget) event)
  (cond
    ...
    ((typep event 'key-event)
     (%input-handle-key-event widget event))))
